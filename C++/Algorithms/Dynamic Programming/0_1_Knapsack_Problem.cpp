...
public:
  ...
  int max_knapsack_value_iterative(const int capacity,
    ...
    std::vector<std::vector<int>> max_value(n + 1,
    ...
    for (int_fast64_t i{}; i < items + 1; ++i) {
      for (int_fast64_t j{}; j < capacity + 1; ++j) {
        if (i == 0 || j == 0) {
          ...
          int profit1 = values[i - 1] + max_value[i - 1][j - weights[i - 1]];
  ...
  int max_knapsack_value_recursive(const int capacity,
    ...
    if (weights[items - 1] > capacity) {
      return max_knapsack_value_recursive(capacity, weights, values, items - 1);
    } else
      return std::max(
          values[items - 1] +
              max_knapsack_value_recursive(capacity - weights[items - 1],
                                           weights, values, items - 1),
          max_knapsack_value_recursive(capacity, weights, values, items - 1));
  }
};
}; // namespace Dynamic_Programming
...
void test2() {
  ...
  assert(k1.max_knapsack_value_recursive(capacity1, weight1, value1, items1) ==
         220);
  std::cout << "TEST CASE 1 : PASSED" << std::endl;
  ...
  assert(k2.max_knapsack_value_recursive(capacity2, weight2, value2, items2) ==
         36);
  std::cout << "TEST CASE 2 : PASSED" << std::endl;
}
